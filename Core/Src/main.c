...
/* USER CODE BEGIN PV */
osThreadId LoaderHandle;
osThreadId RendererHandle;
/* USER CODE END PV */
...
void StartDefaultTask(void const * argument);
void StartTask02(void const * argument);
void loader(void const * argument);
...
QueueHandle_t queue;
EventGroupHandle_t StlEventGroup;
...
point3d_t camera = {0.0,0.0,1.0};
point3d_t light = {1.0,0.0,0.0};

uint8_t buttonFlag = 0;
uint8_t MeshRendering = 0;
...
//  osThreadDef(loader, loader, osPriorityLow, 0, 4096);
//  LoaderHandle = osThreadCreate(osThread(loader), NULL);
...
void draw(obj2d_t* object){
	BSP_LCD_SetTextColor(LCD_COLOR_RED);
	for(size_t i = 0; i<object->point_len; i++){
		BSP_LCD_FillCircle(object->points[i].x, object->points[i].y, 10);
		//BSP_LCD_DrawPixel(object->points[i].x,object->points[i].y,LCD_COLOR_RED);
	}
	BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
...
void loader(void const * argument){
	...
	res=f_mount(&FatFs, SDPath, 0);
	if(res != FR_OK){
		while(1);
	}
	res = f_open(&file, "cube.stl",FA_READ );
	if(res != FR_OK){
		while(1);
	}
...
void StartDefaultTask(void const * argument)
{
  /* USER CODE BEGIN 5 */
	...
	float buffer_x[5] = {0};
	float buffer_y[5] = {0};
	int current_buff_pos = 0;
	float angle_x,angle_y;
	static float G = 9.81;
  /* Infinite loop */
  for(;;)
  {
	if(current_buff_pos > ROLLING_AVG_SIZE-1) current_buff_pos = 0;

	MPU6050_Read_Accel(&hi2c1,&imuData);
	...
	//calculate angles
	buffer_x[current_buff_pos]=angle_x;
	buffer_y[current_buff_pos]=angle_y;
	...
	if(sum_x != 0)
		angle_x = sum_x/ROLLING_AVG_SIZE;
	else
		angle_x = 0;

	if(sum_y != 0)
		angle_y = sum_y/ROLLING_AVG_SIZE;
	else
		angle_y = 0;
	...
	xQueueSend(queue,packet,100);osThreadId RendererHandle;
...
/**
* @brief Function implementing the Renderer thread.
* @param argument: Not used
* @retval None
*/
...
void StartTask02(void const * argument)
{
  /* USER CODE BEGIN StartTask02 */
	...
	__disable_irq();
	obj3d_t reference_cube = cube;
	obj3d_t* cube3d = malloc(sizeof(cube)); //TODO malloc
	obj2d_t* projection=allocProcjection(cube3d->point_len, cube3d->edge_len);
	__enable_irq();

	cube3d->edge_len=cube.edge_len;
	cube3d->point_len=cube.point_len;
	cube3d->points = malloc(sizeof(point3d_t)*cube3d->point_len);
	cube3d->edges = malloc(sizeof(point3d_t)*cube3d->edge_len);
  ...
  for(;;){
	  /*
	  memcpy(cube3d->edges,reference_cube.edges,sizeof(edge)*reference_cube.edge_len);
	  memcpy(cube3d->points,reference_cube.points,sizeof(point3d_t)*reference_cube.point_len);

	  if(xQueueReceive(queue, (void*)packet, 10) == pdTRUE){
		  angle_x=packet[0]*gain;
		  angle_y=packet[1]*gain;
	  ...
	  rotate_obj(cube3d, (angle_x), (angle_y), 0); //rotate oncecamera

	  render_object(cube3d,projection);
